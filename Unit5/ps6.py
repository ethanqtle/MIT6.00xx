...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        '''
        shiftDict = {}
        currentStr = string.ascii_lowercase
        strLen = len(currentStr)
        for i in range(strLen):
            l = currentStr[i]
            shiftL = currentStr[(i + shift) % strLen]
            shiftDict[l] = shiftL
            shiftDict[l.upper()] = shiftL.upper()
        return shiftDict
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        bestPair = (None, None)
        for s in range(26):
            decrypt_msg = self.apply_shift(26-s)
            validCount = 0
            for w in decrypt_msg.split():
                if is_word(self.valid_words, w):
                    validCount += 1
            if bestPair[0] is None or validCount > bestPair[0]:
                bestPair = (validCount, s)
        bestShift = (26 - bestPair[1]) % 26
